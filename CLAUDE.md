...
and claude-listens. As of 2026-09-17 slices 1 to 3 are built: the hook
listener, Apple voices, a playback queue with a stop hotkey, one LLM that
writes a persona preamble and compresses long replies, and a settings
window with a master on/off switch. Listening and further speech providers
are still to come. README.md says what the app does; this file says how we
work on it.
...
4. `ant show blether-Ed6UZ`. The cold review of slices 1 and 2: what was
   ...
   up yet.
5. The `/swift` skill, if it is installed (`~/.claude/skills/swift/SKILL.md`).
...
6. Only if you need the history and have the sibling checkouts:
...
- `Sources/Blether/`: `BletherApp.swift` wires everything at launch.
  ...
  `PlaybackQueue` plays them one at a time. `Settings/` holds the
...
- `Tests/BletherTests/`: XCTest, one file per source file, fakes under
  `Support/`. `make test` runs them; the build is warning-free under strict
...
- Record load-bearing decisions in ant as you go, dated, marked as the
  user's decision or as a lean. Track work in ait. When a slice's tasks
  are written, gate the distillation with the user and run the amnesia
  checker on the deepest ones; it has paid for itself every time.
